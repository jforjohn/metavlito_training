...
"""
Creidits to https://github.com/Spenhouet/tensorboard-aggregator
"""

def extract(dpaths, subpath):
    scalar_accumulators = [EventAccumulator(str(Path(dpath) / subpath)).Reload(
    ).scalars for dpath in dpaths]
...
def aggregate_to_csv(dpath, aggregation_ops, extracts_per_subpath):
    col_names = [aggregation_op.__name__ for aggregation_op in aggregation_ops]
    ...
    for subpath, all_per_key in extracts_per_subpath.items():
        for key, (steps, wall_times, values) in all_per_key.items():
            ...
            write_csv(path, subpath, key, dpath.name, aggregations, steps, col_names)

def summary2csv(dpath, _, extracts_per_subpath):
    path = FOLDER_NAME / dpath.name
    for subpath, all_per_key in extracts_per_subpath.items():
        for key, (steps, wall_times, values) in all_per_key.items():
            sum_items = [np.array(item).reshape(-1)
                         for item in [wall_times, steps, values]]
            write_csv(path, subpath, key, dpath.name, sum_items, np.arange(len(steps)), ['Wall time', 'Steps', 'Value']) 
...
def write_csv(path, subpath, key, fname, values, index, col_names):
    if not path.exists():
        os.makedirs(path)

    #file_name = get_valid_filename(key) + '-' + get_valid_filename(subpath) + '-' + fname + '.csv'
    file_name = get_valid_filename(key) + '-' + get_valid_filename(subpath) + '.csv'
    df = pd.DataFrame(np.transpose(values), index=index, columns=col_names)
...
def aggregate(dpath, output, subpaths):
    ...
    if output in ['summary', 'csv']:
      dpaths = [str(dpath / dname) for dname in os.listdir(dpath) if dname != FOLDER_NAME]
      extracts_per_subpath = {subpath: extract(dpaths, subpath) for subpath in subpaths}
    elif output in ['sum2csv']:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--path", type=str, help="main path for tensorboard files", default='summaries')#os.getcwd())
    parser.add_argument("--savepath", type=str, help="directory to save the results", default='csv_summaries')
    parser.add_argument("--subpaths", type=param_list, help="subpath sturctures", default=['val', 'train'])
    parser.add_argument("--output", type=str, help="aggregation can be saves as tensorboard file (summary) or as table (csv) or sum2csv", default='sum2csv')
    ...
    output = args.output
    print('Functionality', output)

    if not path.exists():
        raise argparse.ArgumentTypeError("Parameter {} is not a valid path".format(path))

    if not savepath.exists():
        raise argparse.ArgumentTypeError("Parameter {} is not a valid path".format(savepath))
    ...
    if output not in ['summary', 'csv', 'sum2csv']:
    ...
    if output in ['summary', 'csv']:
      ...
      for sum_dir in os.listdir(path):
        aggregate(Path(path / sum_dir), output, args.subpaths)